...
namespace Eshva.Poezd.KafkaCoupling.UnitTests
{
  public class given_utf8_byte_string_header_parser
  {
    ...
    public void when_utf8_string_encoded_as_bytes_parsed_it_should_produce_original_string()
    {
      ...
      var parser = new Utf8ByteStringHeaderValueParser();

      var value = parser.Parser(Encoding.UTF8.GetBytes(expectedValue));
    ...
    public void when_zero_length_byte_array_supplied_it_should_return_empty_string()
    {
      ...
      var value = parser.Parser(new byte[0]);

      value.Should().BeEmpty($"it is the contract of {nameof(IHeaderValueParser)}");
    }
    ...
    public void when_null_header_value_supplied_it_should_return_empty_string()
    {
      ...
      var value = parser.Parser(value: null);
